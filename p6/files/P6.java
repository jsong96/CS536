...
/**
 * %This is the main program to test the C-- parser.%
 *
 * There should be 2 command-line arguments: 1. the file to be parsed 2. the
 * output MIPS file
 *
 * The program opens the two files, creates a scanner and a parser, and calls
 * the parser. If the parse is successful, then it will call name analysis and
 * type checking routines. If there is no error at the end, it will generate
 * MIPS code to the output file.
 */
public class P6 {
	FileReader inFile;
	...
	/**
	 * P6 constructor for client programs and testers. Note that users MUST invoke
	...
	private P6(String[] args) {
		// Parse arguments
		if (args.length < 2) {
			String msg = "please supply name of the input file " + "and name of file for assembly output.";
			pukeAndDie(msg);
		}
